...
template<typename T, glm::precision P>
glm::tquat<T, P> quat_fromEulerXYZ(const glm::tvec3<T, P> & v) {
	const T x = v.x;		const T y = v.y;		const T z = v.z;
	const T sx = sin(x/2);	const T sy = sin(y/2);	const T sz = sin(z/2);
	const T cx = cos(x/2);	const T cy = cos(y/2);	const T cz = cos(z/2);
	return glm::tquat<T, P>(cx*cy*cz + sx*sy*sz,
							sx*cy*cz - cx*sy*sz,
							cx*sy*cz + sx*cy*sz,
...
template<typename T, glm::precision P>
glm::tquat<T, P> quat_fromEulerZYX(const glm::tvec3<T, P> & v) {
	const T x = v.x;		const T y = v.y;		const T z = v.z;
	const T sx = sin(x/2);	const T sy = sin(y/2);	const T sz = sin(z/2);
	const T cx = cos(x/2);	const T cy = cos(y/2);	const T cz = cos(z/2);
	return glm::tquat<T, P>(cx*cy*cz - sx*sy*sz,
